...
def get_url_content(url, timeout=30):
    ...
    # 彻底清理 URL：去除空格、换行、引号、反引号以及可能多出来的转义字符
    clean_url = url.strip().strip("'").strip('"').strip("`").strip()
    
    # 针对 wttr.in 的优化：如果用户没有指定格式，自动加上 ?format=4 (单行简洁格式)
    # 这能极大减少数据量，提高成功率和 LLM 处理速度
    if "wttr.in" in clean_url and "?" not in clean_url:
        clean_url += "?format=4"

    # 模拟 curl 请求头，对于 wttr.in 这种服务，curl 头会返回更简洁的文本
    headers = {
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            return json.dumps({"content": response.text.strip()})
        except requests.exceptions.Timeout:
            if attempt < max_retries:
                continue
            return json.dumps({"error": f"The request timed out after {timeout}s. The server might be too slow."})
