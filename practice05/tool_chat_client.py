...
6. 网络访问 (get_url_content):
   ...
   - 参数: url (string, 必须是完整的 URL。禁止包含空格、换行、反引号或引号)
   - 技巧: 查询天气建议优先使用 `https://wttr.in/城市名`（例如 `https://wttr.in/Beijing`）。系统会自动处理编码和格式，返回清晰的中文天气信息。
7. 获取当前日期 (get_current_date):
   - 描述: 获取系统当前真实日期。
8. 查询文档仓库 (anythingllm_query):
   - 描述: 当用户提到“文档仓库”、“文件仓库”、“仓库”或需要从知识库检索信息时，使用此工具。
   - 参数: message (string, 用户的查询关键词或问题)

当你需要调用工具时，请严格按照以下 JSON 格式输出，不要包含任何额外的字符（如反引号）：
...
def main():
    ...
    chat_history = [
        {"role": "system", "content": "你是综合 AI 助手，具备实时联网、文件操作和文档仓库查询能力。当用户提到‘文档仓库’、‘文件仓库’、‘仓库’时，请调用 anythingllm_query 工具。"}
    ]
